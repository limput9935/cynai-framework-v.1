...
async function fetchMarketcap(agentName, count, term) {
    if (count > 30)
        count = 30;
    const query = `
  query MyQuery {
...
async function fetchTopHolders(agentName, mintAddress) {
    const query = `
  query MyQuery {
...
async function fetchFirstTopBuyers(agentName, mintAddress, count) {
    const query = `
  query MyQuery {
    Solana {
      DEXTrades(
...
async function fetchTrendingTokens(agentName) {
    const query = `
query MyQuery {
  Solana {
    DEXTradeByTokens(
